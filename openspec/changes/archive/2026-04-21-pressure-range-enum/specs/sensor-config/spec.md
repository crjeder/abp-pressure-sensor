...
### Requirement: ParseError includes InvalidUnit and InvalidType
**Reason**: The 5-character pressure+type code is now validated atomically as a `PressureRange`. Distinguishing between "wrong unit letter" and "wrong type letter" is no longer meaningful.
**Migration**: Replace matches on `ParseError::InvalidUnit` or `ParseError::InvalidType` with `ParseError::InvalidPressureRange`.
